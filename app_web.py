Ho preparato la nuova versione con interfaccia Modern Web UI, mantenendo Streamlit.

Nuova versione pronta
DACHSER Packer – Modern UI

Python · Streamlit · Motore Planner Beta 5

Le principali novità:

Header DACHSER blu, più compatto.

Inserimento colli riorganizzato.

Risultati in stile dashboard, con barra di saturazione.

Piano di carico orizzontale a tutta larghezza.

Elenco colli compatto, raggruppato per scarico.

Grafica rinnovata, con colori e spaziature uniformi.

Scarica DACHSER Packer Modern UI

Funzionalità preservate: motore Beta 5, rotazione, sovrapponibilità, multi-drop, importazione Excel e generazione PDF.

Ho verificato la sintassi e superato i test automatizzati su calcolo, rotazione, overflow e PDF. Non ho ancora potuto verificare l'aspetto dell'interfaccia in un browser con Streamlit. 

Per avviarla:

streamlit run DACHSER_Packer_Vicenza_Modern_UI.py

Apri app_web.py nel repository GitHub, clicca sulla matita, elimina il contenuto attuale e incolla il codice completo qui sotto. Mantieni il nome app_web.py e salva con Commit changes.

Il codice include il motore Beta 5 e la nuova interfaccia Streamlit.
...
Il file requirements.txt deve contenere almeno queste dipendenze:
...
Dopo il commit, Streamlit Community Cloud dovrebbe rilevare l'aggiornamento e riavviare l'applicazione.

manca il pulsante + e - vicino alla quantità dei pallet in fase di inserimento. poi metterei la parte con i pallet inseriti vicino al campo di inserimento e non così in basso

Ho aggiornato la versione Modern UI mantenendo Streamlit e il motore Beta 5.

Le due modifiche richieste sono pronte:

Aggiunti i pulsanti − e + accanto alla quantità dei pallet. Il valore può ancora essere digitato manualmente e non scende sotto 1.

Spostato l'elenco Colli inseriti immediatamente sotto il modulo di inserimento, nella stessa colonna. Restano disponibili modifica, eliminazione e svuotamento.

Versione aggiornata
DACHSER Packer – Modern UI v2

Scarica il codice completo app_web.py

Il file è già pronto per sostituire quello presente su GitHub.

Ho verificato che motore di calcolo e PDF siano invariati. I test su pulsanti, rotazione, multi-drop e overflow sono passati. 

scrivi il codice qui per copia incolla

Ecco il codice completo aggiornato, con i pulsanti − / + per la quantità e l'elenco dei pallet immediatamente sotto il modulo di inserimento.

Puoi anche copiare direttamente il contenuto del
file app_web.py 
già pronto per GitHub.
